...
const EventAudioUpload = (props) => {
  ...
  // Initialize modals based on action type and recordings
  useEffect(() => {
    console.log("🔍 Modal initialization useEffect triggered:", {
      autoOpenModal: props.autoOpenModal,
    ...
    if (props.autoOpenModal && !isLoading && recordings.length > 0) {
      console.log("✅ Conditions met, initializing modal for:", props.actionType);

      if (props.actionType === "new-recording") {
        console.log("📝 Setting showUploadModal to true");
        setShowUploadModal(true);
      } else if (props.actionType === "remap-session") {
        // For remap-session, automatically select the first mapped recording and open session modal
        const mappedRecordings = recordings.filter((rec) => rec.session);
        console.log("🔄 Remap session - mapped recordings:", mappedRecordings);

        if (mappedRecordings.length > 0) {
          console.log("✅ Found mapped recordings, setting up session modal");
          setSelectedRecording(mappedRecordings[0]);
          setIsRemapping(true);
          setShowSessionModal(true);
        } else {
          console.log("❌ No mapped recordings, showing recording selector");
          // No mapped recordings available, show message
          setShowRecordingSelector(true);
        }
      } else if (props.actionType === "map-session") {
        console.log("🔗 Map session action triggered");

        // If specific recording data is provided, skip selector and directly show session modal
        if (props.selectedRecordingData?.data) {
          console.log("✅ Selected recording data provided, showing session modal directly");
          const recordingData = props.selectedRecordingData.data;

          // Find the full recording object from the recordings list
          const recording = recordings.find((rec) => rec.id === recordingData._id);
          ...
          if (recording) {
            ...
            console.log("❌ Recording not found in list, showing selector");
            setShowRecordingSelector(true);
          }
        } else {
          console.log("🔗 No specific recording, showing recording selector");
          // For map-session without specific recording, show recording selector
          setShowRecordingSelector(true);
        }
      } else if (props.actionType === "view-transcript") {
        console.log("👁️ View transcript action triggered");

        // If specific recording data is provided, skip selector and directly show transcript
        if (props.selectedRecordingData?.data) {
          console.log("✅ Selected recording data provided, showing transcript directly");
          const recordingData = props.selectedRecordingData.data;
          ...
          if (recording && recording.status === "processed") {
            ...
            console.log("❌ Recording not found or not processed, showing selector");
            setShowRecordingSelector(true);
          }
        } else {
          console.log("👁️ No specific recording, showing recording selector");
          // For view-transcript without specific recording, show recording selector
          setShowRecordingSelector(true);
        }
      }
    } else {
  ...
  }, [props.autoOpenModal, props.actionType, props.selectedRecordingData, isLoading, recordings]);
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // If modal was auto-opened and we have a callback, close the parent popup too
        if (props.autoOpenModal && props.onModalClose) {
          props.onModalClose();
        }
      }
    } catch (error) {
  ...
  const mapSession = async (sessionId) => {
    if (selectedRecording && sessionId) {
      ...
      // If this was triggered from top-level actions, close the parent modal too
      if (props.autoOpenModal && props.onModalClose) {
        props.onModalClose();
      }
    }
  ...
  // If auto-opening modal, don't show the page content
  if (props.autoOpenModal) {
    return (
      <>
                ...
                <div className="flex justify-center">
                  <div className="flex gap-2 p-1 bg-bg-weak rounded-xl">
                    <button
                      ...
                      className={`px-6 py-2.5 rounded-lg font-medium transition-all duration-200 ${mode === "upload" ? "bg-bg-white text-text-main shadow-sm" : "text-text-sub hover:text-text-main"}`}
                    >
                      Upload Audio
                    </button>
                    <button
                      onClick={() => setMode("record")}
                      className={`px-6 py-2.5 rounded-lg font-medium transition-all duration-200 ${mode === "record" ? "bg-bg-white text-text-main shadow-sm" : "text-text-sub hover:text-text-main"}`}
                    >
                      Record Audio
                    </button>
                  </div>
                  ...
                  {mode === "upload" ? (
                        ...
                        <div className="space-y-6">
                          ...
                          <div className="mb-6">
                            ...
                            <div className="w-full bg-bg-weak rounded-full h-2.5">
                              <div className="bg-primary-base h-2.5 rounded-full transition-all duration-300" style={{ width: `${uploadProgress}%` }}></div>
                            </div>
                          </div>
                        </div>
                      ) : (
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-center gap-3">
                        {!isRecording ? (
                          <Button value="Start Recording" icon="mic" ClickEvent={startRecording} type="primary" align="bg-primary-base hover:bg-primary-dark text-white px-6 py-3" />
                        ) : (
                          <>
                            <Button
                            ...
                            />
                            <Button value="Stop" icon="stop" ClickEvent={stopRecording} type="primary" align="bg-state-error hover:bg-red-600 text-white px-6 py-3" />
                          </>
                        )}
                      </div>
                        ...
                        <div className="p-4 bg-bg-weak rounded-lg space-y-4">
                          ...
                          <div className="flex gap-3 justify-center">
                            <Button
                              ...
                              type="primary"
                              align={`bg-state-success hover:bg-green-600 text-white px-6 py-3 ${isUploading ? "opacity-50 cursor-not-allowed" : ""}`}
                            />
                          </div>
                        </div>
                      )}
        ...
        {/* Recording Selector Modal - For Map/Remap/View actions */}
        {showRecordingSelector && (
          <>
            {console.log("🎯 Rendering Recording Selector Modal:", { showRecordingSelector, actionType: props.actionType })}
            <div
            ...
            >
              <div
                className="bg-white p-6 rounded-xl max-w-4xl w-full mx-4 shadow-xl max-h-[80vh] overflow-y-auto"
                style={{
              ...
              >
                <div className="mb-6">
                  <h3 className="text-lg font-semibold text-text-main mb-2">
                    {props.actionType === "map-session" ? "Map Recordings to Session" : props.actionType === "remap-session" ? "Remap Recordings to Session" : "View Recording Transcripts"}
                  </h3>
                  <p className="text-sm text-text-sub">
                    {props.actionType === "map-session"
                      ? "Select recordings to map to sessions"
                      : props.actionType === "remap-session"
                        ? "Select recordings to remap to different sessions"
                        : "Select recordings to view their transcripts"}
                  </p>
                </div>
                  ...
                  <div className="space-y-3 mb-6">
                    {(() => {
                      const filteredRecordings = recordings.filter((rec) => {
                        if (props.actionType === "map-session") return !rec.session;
                        if (props.actionType === "remap-session") return rec.session;
                        if (props.actionType === "view-transcript") return rec.status === "processed";
                        return true;
                      });
                      ...
                      return filteredRecordings.map((recording) => (
                        <div key={recording.id} className="p-4 border border-stroke-soft rounded-xl bg-bg-white hover:border-stroke-sub hover:shadow-sm transition-all duration-200">
                          <div className="flex justify-between items-center">
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div className="flex-1">
                                  ...
                                  <div className="flex items-center gap-2 flex-wrap mb-2">
                                    ...
                                    {!recording.session && recording.status !== "processed" && recording.status !== "failed" && !(recording.status === "processing" && recording.progress > 0) && (
                                      <span className="px-2 py-1 rounded-full text-xs font-medium bg-blue-50 text-blue-700 border border-blue-200">Uploaded</span>
                                    )}
                                  </div>
                            ...
                            {/* Action Buttons - Same as standalone page */}
                            <div className="flex items-center gap-2">
                              {/* Map/Remap Session Button */}
                              {props.actionType === "map-session" || props.actionType === "remap-session" ? (
                                <Button
                              ...
                              {props.actionType === "view-transcript" && recording.status === "processed" && (
                                <Button
                        ...
                        filteredRecordings.length === 0 && (
                          <div className="text-center py-8">
                            <p className="text-text-sub">
                              {props.actionType === "map-session"
                                ? "No unmapped recordings available"
                                : props.actionType === "remap-session"
                                  ? "No mapped recordings available"
                                  : "No processed recordings available"}
                            </p>
                          </div>
                        )
                      );
        ...
        {showSessionModal && selectedRecording && (
          <>
            {console.log("🎯 Rendering Session Selection Modal:", { showSessionModal, selectedRecording: selectedRecording?.title, isRemapping })}
            <div
              className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center"
              style={{
                position: "fixed",
            ...
            >
              <div
                className="bg-bg-white p-6 rounded-xl max-w-md w-full mx-4 shadow-xl"
                style={{
                  backgroundColor: "white",
                  zIndex: 10000,
                  position: "relative",
              ...
              >
                <div className="mb-6">
                  <h3 className="text-lg font-semibold text-text-main mb-2">{isRemapping ? "Remap Session" : "Map to Session"}</h3>
                ...
                <div className="mb-6">
                  <CustomSelect
                    ...
                    required={true}
                    apiType="JSON"
                    selectApi={[{ id: "none", value: "None" }, ...sessions.map((s) => ({ id: s.id, value: s.value }))]}
  ...
  return (
    <RowContainer className="data-layout">
      ...
      <ButtonPanel className="custom">
        <div className="flex items-center gap-3">{/* Search and filter can be added here if needed */}</div>

        <div className="flex items-center gap-3">
          <Button value="New Recording" icon="add" ClickEvent={() => setShowUploadModal(true)} type="primary" align="bg-primary-base hover:bg-primary-dark text-white" />
        </div>
      </ButtonPanel>

      {/* Recordings List */}
      {recordings.length === 0 ? (
        <NoDataFound shortName="Recordings" icon="music" addPrivilege={true} addLabel="New Recording" isCreatingHandler={() => setShowUploadModal(true)} />
      ) : (
      ...
      {/* Upload Modal - Only show when not auto-opened */}
      {showUploadModal && !props.autoOpenModal && (
        <PopupView
          customClass="medium"
          popupData={
            <div className="space-y-6 p-2">
