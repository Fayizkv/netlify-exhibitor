    ...
    <FileContainer className={`${props.customClass ?? "full"} ${props.dynamicClass ?? ""}`}>
      ...
      <div>
        <div className="w-[160px] h-[90px]">
          {video ? (
            <video style={{ width: "100%", height: "100%" }} className="contain" controls src={previewVideo ? previewVideo : video ? import.meta.env.VITE_CDN + video : noimage} />
          ) : (
            <video style={{ width: "100%", height: "100%" }} className="contain" controls src={previewVideo ? previewVideo : noimage} />
          )}
        ...
        <div>
          ...
          <SubPageHeader
            dynamicClass="custom"
            line={false}
            description={props.value?.length > 0 ? `File size: ${size} <br /> Supported file types: MP4, WebM, Ogg` : `File size: Up to 50MB <br /> Supported file types: MP4, WebM, Ogg`}
          />
          <Footnote {...props} />
