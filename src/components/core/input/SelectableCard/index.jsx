...
const CardWrapper = styled.label`
  ...
  max-width: 600px;
  width: 100%;
  padding: ${(props) => (props.$cardType === "compact" ? "9px" : "16px")};
  background: white;
  border: 1px solid ${(props) => (props.$checked ? appTheme.primary.base : "#E9ECEF")};
  border-radius: ${(props) => (props.$cardType === "compact" ? "8px" : "12px")};
...
const StatusIndicator = styled.div`
  ...
  width: 20px;
  height: 20px;
  border: 1px solid ${(props) => (props.$checked ? "#4263EB" : "#CED4DA")};
  border-radius: ${(props) => (props.type === "radio" ? "50%" : "4px")};
  background: ${(props) => (props.$checked ? "#4263EB" : "transparent")};
  transition: all 0.2s ease;
...
const SelectableCard = ({ id, type = "radio", name, value, checked = false, onChange, option, disabled = false, compact = false }) => {
  ...
  return (
    <CardWrapper
      ...
      $cardType={compact ? "compact" : "normal"}
