...
const SearchMenu = ({ tabs, onTabChange }) => {
  ...
  const handleSearch = (term) => {
    ...
    const results = allTabItems.filter((item) => item.label.toLowerCase().includes(term.toLowerCase()) || (item.parentLabel && item.parentLabel.toLowerCase().includes(term.toLowerCase())));
  ...
  return (
    <div className="relative" ref={searchRef}>
        ...
        <div className="flex items-center transition-all duration-200">
          <div className="relative flex items-center border border-gray-200 rounded-lg bg-white h-9 min-w-[220px]">
            ...
            <input
              ...
              className="pl-10 pr-8 py-1.5 w-56 md:w-72 bg-transparent text-gray-700 placeholder-gray-400 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-300 text-sm"
              value={searchTerm}
      ...
      {isSearchOpen && searchResults.length > 0 && (
        <div
          className="absolute top-[38px] left-0 w-full mt-1 bg-white rounded-lg shadow-lg max-h-60 overflow-auto border border-gray-100 z-[1001] min-w-[220px]"
