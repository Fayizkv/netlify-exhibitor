      ...
      {images.length > 0 && (
        <Progess viewMode={viewMode}>
          ...
          <div className="buttons">
            ...
            {!isUploading && !isStopping && overallStatus !== "completed" && (
              <Button
                value={isFailed ? "Retry" : isStopped ? "Resume Upload" : "Start Upload"}
                ClickEvent={retryFailedUploads}
                type={isFailed ? "error" : isStopped ? "primary" : "primary"}
                icon={isFailed ? "reload" : isStopped ? "play" : "play"}
              />
            )}
