...
// AV Team URL Render Component - dynamically imported
const AvTeamUrlRenderComponent = React.lazy(() => import("../../project/pages/event/attributes/avTeam/AvTeamUrlRenderComponent"));
...
const checkParentCondition = (attribute, parentData) => {
  ...
  const finalResult = resultValue === "enabled" ? true : false;
  return finalResult;
};
...
const ListItems = React.memo(
  ({
    ...
    // Sync users data to local state when listSort is enabled
    useEffect(() => {
      if (listSort && users) {
        setLocalUsers(users);
      }
      ...
      const initialCheck = async () => {
        if (attributes.length > 0) {
          ...
          const promises = attributes.map(async (item) => {
            if (item.default?.source === "parent" && item.default?.field) {
              const parentValue = parentData[item.default.field];

              if (item.default.type === "match" && item.default.dataset) {
                const matchedItem = item.default.dataset.find((d) => d.match === parentValue);
                addValuesTemp.addValues[item.name] = matchedItem ? matchedItem.value : (item.default.fallback ?? null);
              } else {
                addValuesTemp.addValues[item.name] = parentValue ?? item.default.fallback ?? null;
              }
            } else {
              if (item.type === "checkbox" || item.type === "toggle") {
                ...
                if (item.default?.source === "parent" && item.default?.field) {
                  addValuesTemp.addValues[item.name] = moment(parentData[item.default.field]).isValid() ? moment(parentData[item.default.field]).toISOString() : "";
                } else {
    ...
    useEffect(() => {
      ...
      if (currentIndex === 0 && users?.counts) {
        setCounts(users.counts);
      }
      if(currentIndex === 0 && users?.filterCount)
      {
    ...
    // Handle drag end for list sorting
    const handleDragEnd = useCallback(
      async (event) => {
        ...
        const oldIndex = localUsers.response.findIndex(item => item._id === active.id);
        const newIndex = localUsers.response.findIndex(item => item._id === over.id);
        ...
        const reorderedItems = arrayMove(localUsers.response, oldIndex, newIndex);
        setLocalUsers({
          ...localUsers,
          response: reorderedItems
        });

        // Calculate actual positions considering pagination
        const startPosition = (pageNumber - 1) * perPage;
        
        // Update positions in the reordered array
        const updatedItemsWithPositions = reorderedItems.map((item, index) => ({
          _id: item._id,
          position: startPosition + index + 1, // 1-based position
        }));
        ...
        try {
          ...
          if (response.status === 200) {
            ...
            // Revert on error
            setLocalUsers(users);
            toast.error(response.customMessage || "Failed to update order");
          }
        } catch (error) {
      ...
      [localUsers, users, api, toast, refreshView, currentIndex, pageNumber, perPage]
    );
    ...
    const setEditingData = (value, updateValuesTemp) => {
      console.log("setEditingData", value);
          ...
          if (item.type === "checkbox" || item.type === "toggle") {
            ...
            updateValuesTemp[item.name] =
              typeof itemValue === "undefined"
                ? ""
                : typeof itemValue === "string" || typeof itemValue === "number" || typeof itemValue === "boolean"
                  ? itemValue
                  : value[item.name]?._id
                    ? value[item.name]._id
                    : "";
    ...
    const submitHandler = async (data) => {
      ...
      const parentDataItem = { ...parents, _id: null };
      ...
      await postData({ ...parentDataItem, ...filteredData }, currentApi)
        .then((response) => {
    ...
    // Sortable wrapper for TableRowWithActions when listSort is enabled
    const SortableTableRow = React.memo(({ id, slNo, attributes, data, inlineSelectData }) => {
      const {
      ...
      return (
        <TableRowWithActions
          ...
          inlineSelectData={inlineSelectData}
          data={data}
    ...
    const TableRowWithActions = React.memo(({ attributes, data, slNo, inlineSelectData = {}, dragHandleProps = null, sortableProps = null }) => {
      ...
      const onClick = () => {
        if (!signleRecord) {
          if (itemOpenInSlug) {
            ...
            setOpenData({ actions, attributes, data, openPageContent });
        ...
        case "table":
          return (
            <TrView
              ...
              key={`${shortName}-${slNo}`}
            >
              {/* Drag handle column when listSort is enabled */}
              {listSort && dragHandleProps && (
                <TdView 
                  className="drag-handle" 
                  style={{ width: '40px', padding: '8px', cursor: 'grab' }}
                  onClick={(e) => e.stopPropagation()}
                  onPointerDown={(e) => e.stopPropagation()}
                >
                  <div {...dragHandleProps} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', touchAction: 'none' }}>
                    <GripVertical size={16} className="text-text-sub" />
                  </div>
                </TdView>
              )}
              ...
              {attributes.map((attribute, index) => {
                if (attribute.view && (attribute.tag ?? false) && checkParentCondition(attribute, parentData)) {
                  try {
                    ...
                    if (attribute.showSubItem) {
                      itemValue =
                        attribute.collection?.length > 0 && attribute.showItem?.length > 0 ? (data[attribute.collection][attribute.showItem][attribute.showSubItem] ?? "") : data[attribute.name];
                    }
                    let dynamicClass = "";
                                ...
                                {attribute.render ? (
                                  ...
                                  <ImageRow>
                                    {attribute.image?.generateTextIcon ? (
                                      <ProfileImage
                                        imageUrl={attribute.image.collection?.length > 0 ? data[attribute.image.collection]?.[attribute.image.field] : data[attribute.image.field]}
                                        title={value}
                                        customProfileSource={customProfileSource}
                                        onImageClick={(src) => setShowImage({ src })}
                                        theme={themeColors}
                                        className="w-auto"
                                      />
                                    ) : (
                                      <img
                                        ...
                                        src={
                                          import.meta.env.VITE_CDN + (attribute.image.collection?.length > 0 ? data[attribute.image.collection]?.[attribute.image.field] : data[attribute.image.field])
                                        }
                                        alt={attribute.name}
                                      />
                                    )}
                                    <div>
                                      ...
                                      {attribute.description && (
                                        <div>
                                          {getValue(
                                            attribute.description,
                                            attribute.description.collection?.length > 0 ? data[attribute.description.collection]?.[attribute.description.field] : data[attribute.description.field]
                                          )}
                                        </div>
                                      )}
                                    </div>
                                  ...
                                  <React.Fragment>
                                    ...
                                    {attribute.statusLabel && (
                                      <StatusLabel
                                        statusLabels={attribute.statusLabel.conditions}
                                        nextLine={attribute.statusLabel.nextLine}
                                        values={data}
                                        size={attribute.statusLabel.size ?? "small"}
                                      ></StatusLabel>
                                    )}
                                    {attribute.description && (
                                      <DescRow>
        ...
        default:
          return (
            <SetTr
              ...
              key={`row-${shortName}-${data._id ?? slNo}`}
            >
              {profileImage && displayColumn !== "triple" && (
                <ProfileImage imageUrl={data[profileImage]} title={titleValue} customProfileSource={customProfileSource} onImageClick={(src) => setShowImage({ src })} theme={themeColors} />
              )}
                      ...
                      <Head
                        onClick={() => {
                          if (itemOpenInSlug) {
                            ...
                            window.location.href = `${pathname}/${data._id}`;
                          } else if (itemOpenMode.type === "edit") {
                            isEditingHandler(data, udpateView, titleValue);
                          } else if (itemOpenMode.type === "open" && openPage) {
                            window.location.hash = "";
                              ...
                              if (attribute.showSubItem) {
                                itemValue =
                                  attribute.collection?.length > 0 && attribute.showItem?.length > 0
                                    ? (data[attribute.collection][attribute.showItem][attribute.showSubItem] ?? "")
                                    : data[attribute.name];
                              }
                                    ...
                                    <Td className={"custom " + dynamicClass} key={index}>
                                      <React.Fragment>
                                        {displayColumn === "triple" ? (
                                          attribute.itemLabel !== undefined && attribute.itemLabel !== null ? (
                                            attribute.itemLabel === "" ? (
                                              ""
                                            ) : (
    ...
    const exportToExcel = async () => {
      ...
      try {
            ...
            .forEach((attribute) => {
              try {
                ...
                switch (attribute.type) {
                  case "image":
                    if (valueToProcess) {
                      if (Array.isArray(valueToProcess) && valueToProcess.length > 0) {
                        // Handle multiple images - show "Click here" for each image
                        const clickTexts = valueToProcess.map((_, index) => `Click here ${index + 1}`);
                        row[attribute.name] = clickTexts.join(", ");
                      } else if (typeof valueToProcess === "string" && valueToProcess.length > 0) {
                        // Handle single image - show "Click here"
                        row[attribute.name] = "Click here";
                      } else {
                        row[attribute.name] = "";
                      }
                    } else {
                  ...
                  case "file":
                    if (valueToProcess) {
                      if (Array.isArray(valueToProcess) && valueToProcess.length > 0) {
                        // Handle multiple files - show "Click here" for each file
                        const clickTexts = valueToProcess.map((_, index) => `Click here ${index + 1}`);
                        row[attribute.name] = clickTexts.join(", ");
                      } else if (typeof valueToProcess === "string" && valueToProcess.length > 0) {
                        // Handle single file - show "Click here"
                        row[attribute.name] = "Click here";
                      } else {
                        row[attribute.name] = "";
                      }
                  ...
                  case "mobilenumber":
                    row[attribute.name] =
                      typeof valueToProcess === "object" && valueToProcess !== null ? `+${valueToProcess.country}${valueToProcess.number}` : `+${data.phoneCode || ""}${valueToProcess}`;
                    break;
                  case "minute":
                  ...
                  case "select":
                    ...
                    break;
                  case "multiSelect":
                    // Use valueToProcess which already handles collection logic
                    const multiSelectArray = valueToProcess; 
                    if (Array.isArray(multiSelectArray)) {
                      if (attribute.apiType === "API") {
                        ...
                        // For CSV, parse the selectApi and match IDs to display values
                        const csvOptions = attribute.selectApi?.split(",").map((itemValue) => ({
                          id: itemValue.trim(),
                        ...
                        row[attribute.name] = multiSelectArray
                          .map((id) => {
                            const option = csvOptions.find((opt) => opt.id?.toString() === id?.toString());
                            return option ? option.value : id?.toString();
                          })
                          .filter(Boolean)
                          .join(", ");
                      } else {
                        // apiType === "JSON" or default
                        row[attribute.name] = multiSelectArray
        ...
        // Add hyperlinks for image and file fields after rows are added
        const filteredAttributes = attributes.filter((attribute) => attribute.export !== false);
        const imageFileAttributes = filteredAttributes.filter((attribute) => attribute.type === "image" || attribute.type === "file");

        if (imageFileAttributes.length > 0) {
          jsonData.forEach((data, rowIndex) => {
            const actualRowNumber = rowIndex + 2; // +2 because row 1 is header
            const row = worksheet.getRow(actualRowNumber);

            imageFileAttributes.forEach((attribute) => {
              try {
                let valueToProcess;
                ...
                if (attribute.collection && data[attribute.collection] !== undefined && data[attribute.collection] !== null) {
                  const collectedObject = data[attribute.collection];
                  if (typeof collectedObject === "object" && attribute.showItem) {
                    const potentialValue = collectedObject[attribute.showItem];
                    valueToProcess = attribute.showSubItem && typeof potentialValue === "object" && potentialValue !== null ? potentialValue[attribute.showSubItem] : potentialValue;
                  } else {
                    valueToProcess = collectedObject;
                  }
                } else {
                  valueToProcess = data[attribute.name];
                }
                ...
                if (valueToProcess) {
                  const columnIndex = filteredAttributes.findIndex((attr) => attr.name === attribute.name) + 1;
                  const cell = row.getCell(columnIndex);

                  // Get the link item (first item for arrays, or the value itself for strings)
                  const linkItem = Array.isArray(valueToProcess) ? valueToProcess[0] : valueToProcess;

                  if (linkItem) {
                    const fullUrl = import.meta.env.VITE_CDN + linkItem;
                    cell.value = {
                ...
                console.error(`Error adding hyperlink for column ${attribute.name}:`, error);
              }
            });
          });
        }
    ...
    const TableRows = React.memo(
      ({ users, attributes, shortName, inlineSelectData }) => {
        ...
        const items = (localUsers || users).response;

        // Render rows without DndContext wrapper (to avoid invalid HTML)
      ...
      (prevProps, nextProps) => {
        ...
        return (
          ...
          prevProps.inlineSelectData === nextProps.inlineSelectData
        );
      }
    );
    ...
    return isSlug ? (
      ...
      <RowContainer
        theme={themeColors}
        className={"data-layout " + viewMode + (fullScreen ? " !fixed top-0 left-0 right-0 bottom-0 z-50 bg-white transition-all duration-300" : "transition-all duration-300")}
      >
        {showTitle && <PageHeader dynamicClass={headerStyle} title={shortName} line={false} description={generateDescription()}></PageHeader>}

        {labels.length > 0 && (typeof MetricTileRender === "function" ? <MetricTileRender labels={labels} data={counts} /> : <MetricTile labels={labels} data={counts} isLoading={shimmerLoader} />)}

        {infoBox && (
          <InfoBox
            icon={infoBox.icon || "info"}
            showIcon={infoBox.showIcon !== false}
            type={infoBox.type || "info"}
            content={
              infoBox.content === "AV_TEAM_URL_BOX" ? (
                <React.Suspense
                  fallback={
                ...
                >
                  <AvTeamUrlRenderComponent eventId={referenceId || parents?.event} />
                </React.Suspense>
              ) : (
                infoBox.content
              )
            }
            closable={infoBox.closable !== false}
            api={api}
            storageKey={infoBox.storageKey}
            title={infoBox.title}
            actionButton={infoBox.actionButton}
            design={infoBox.design || "1"}
          />
        )}
                ...
                <React.Fragment>
                  {filterElements?.left?.map((item, index) => (
                    <FormInput
                      ...
                      {...item}
                      filter={{ ...parents }}
                      onChange={(option, name, type) => filterChange(option, name, type, item.parentFilter ?? false)}
                      showLabel={false}
                      required={false}
                      selectType={item.filterType}
                      footnote={null}
                      addNew={null}
                      type={"select"}
                  ...
                  <div className="sm:flex gap-2">
                    ...
                    {additionalButtons.map((btn) => (
                      <AddButton
                        ...
                        theme={themeColors}
                        onClick={() => {
                          console.log("Additional Button Clicked", referenceId, parents, filterView);
                          btn.onClick(referenceId, parents, filterView);
                        }}
                      >
                    ...
                    {typeof ListItemsRender === "function" ? (
                      ...
                      <TableContaner>
                        {listSort ? (
                          ...
                          >
                            <SortableContext 
                              items={(localUsers || users)?.response?.map(item => item._id) || []} 
                            ...
                            >
                              <TableView theme={themeColors}>
                                {showHeaderRow && (
                                  <thead>
                                    <tr>
                                      {/* Drag handle header when listSort is enabled */}
                                      {listSort && (
                                        <ThView className="drag-handle">
                        ...
                        {initialized && !users && !users?.response && (
                          <NoDataFound
                            setShowBulkUplad={setShowBulkUplad}
                            bulkUplaod={bulkUplaod}
                            shortName={shortName}
                        ...
                        {initialized && users?.response?.length === 0 && (
                          <NoDataFound
                            setShowBulkUplad={setShowBulkUplad}
                            bulkUplaod={bulkUplaod}
                            shortName={shortName}
        ...
        {isOpen && updateValues && (
          <Popup
            ...
            editData={{
              ...
              setMessage: setMessage,
              setLoaderBox: setLoaderBox,
              openPageContent: openData?.openPageContent,
            }}
            popupMode={popupMode}
            showInfo={showInfo}
            popupMenu={popupMenu}
        ...
        {isEditing && (
          <CrudForm
            ...
            formType={"put"}
            updateId={updateId}
            header={`${updateValues.clone === false ? `${shortName}: ` : `Clone ${shortName}: `}  <span style="font-weight:bold">'${updateValues._title}'</span>`}
            formInput={formInput}
            formErrors={errroInput}
            formValues={updateValues}
            submitHandler={updateHandler}
        ...
        {detailView && (
          <Details formMode={formMode} closeModal={closeModal} themeColors={themeColors} setMessage={setMessage} setLoaderBox={setLoaderBox} itemTitle={itemTitle} openData={openData}></Details>
        )}
      ...
      <RowContainer className={"data-layout "} key={date}>
        ...
        {users?.response?.length > 0 && (
          <CrudForm
            subGroupSettings={subGroupSettings}
            formStyle={formStyle}
            parentData={parentData || users?.response[0]}
            {...{
              parentReference: parentReference,
              referenceId: referenceId,
              formMode: formMode,
