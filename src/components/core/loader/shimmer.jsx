...
export const ListTableSkeleton = ({ viewMode = "table", displayColumn = "single", tableColumnCount = 5 }) => {
  ...
  if (viewMode === "list" || viewMode === "subList") {
    return (
      <div className="p-0">
        <div className={displayColumn === "triple" ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" : "grid grid-cols-1"}>
          {displayColumn === "triple" ? [...Array(9)].map((_, idx) => <CardSkeleton key={idx} />) : [...Array(8)].map((_, idx) => <SingleRowSkeleton key={idx} />)}
        </div>
      </div>
    );
  }
...
export const SimpleShimmer = ({ message = "Loading..." }) => (
