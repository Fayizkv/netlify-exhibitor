  ...
  return (
    <div className="flex items-center justify-center w-full text-center">
      <button
        ...
        onClick={onRefresh}
        disabled={isLoading}
        className="text-[12px] inline-flex items-center gap-1 text-gray-600 hover:text-gray-700 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
...
function CustomSelect(props) {
  ...
  const renderList = () => (
    <div>
      <ul style={{ height: "calc(100dvh - 120px)" }} className={`  options ${optionsVisible ? dropdownPosition : "hide"} pr-1 overflow-auto`}>
        {initialized && (
          <>
            {props.search && !props.disableSearch && options.length > 8 && (
              <Search active={true} key={`search-inside${props.key}`} className="select" title="Search" theme={props.theme} placeholder="Search" value={searchValue} onChange={handleChange} />
            )}
            {props.apiSearch && (
              <Search
                active={true}
                key="search-api-2"
                className={"select " + props.customClass}
                title="Search"
                theme={props.theme}
  ...
  const renderDropdownOptions = () => (
    <SelectBox
      key={props.key}
      theme={props.theme}
      className={`${props.dynamicClass === "disabled" ? "hidden" : ""} custom-select ${props.listBox ? "list-box" : ""} ${optionsVisible ? "open" : "close"} ${props.customClass} ${props.dynamicClass}`}
    >
                        ...
                        <TagBox>
                          {props.iconImage && (
                            <ImgBox
                              ...
                              src={
                                import.meta.env.VITE_CDN + (props.iconImage.collection.length > 0 ? (option[props.iconImage.collection]?.[props.iconImage.item] ?? "") : option[props.iconImage.item])
                              }
                            />
                          )}
                          <RowContainer>
  ...
  const reactSelectOptions = () => {
    ...
    return (
      ...
      >
        <div
          className={`${props.dynamicClass?.includes("disabled") ? "hidden" : ""} col-span-12 rounded-md flex flex-col gap-1 w-full min-w-[150px] ${hasFilter ? "w-[max-content] max-w-[200px]" : ""}`}
        >
          {(props.showLabel ?? true) && (
            <div className="flex items-center gap-2 justify-between">
              ...
              {addNew && addNew.attributes?.length > 0 && (
                <div
                  className="text-primary-base rounded-md w-[max-content] add-new-option flex items-center gap-2 p-0 text-[12px] text-primary cursor-pointer border-gray-200"
          ...
          <div className="relative">
            ...
            />
            {hasIcon && (
              <div className={`z-10 absolute left-3 top-[18px] -translate-y-1/2 pointer-events-none ${hasFilter ? "pt-0" : "pt-1"}`}>
                {isLoading ? <Loader className="w-4 h-4 text-gray-500 animate-spin" aria-hidden="true" /> : <GetIcon icon={props.icon} className="w-4 h-4 text-gray-500" />}
              </div>
            )}
          </div>
          <Footnote {...props} />
          ...
          (searchValue.length > 0 ? filteredOptions : options).map((option) => {
            ...
            return (
              <SelectableCard
                ...
                checked={isSelected}
                type="radio"
                compact={props.cardType === "compact"}
                onChange={() => {
                  if (selectedId === option.id) {
                    props.onSelect({ id: defaultValue, value: props.placeholder }, props.id, props.type);
                    setSelectedValue(props.placeholder);
