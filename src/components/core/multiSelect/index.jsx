...
function MultiSelect(props) {
  ...
  const queryParams = useMemo(() => {
    ...
    // Handle updateOn parameter - send arrays as JSON strings
    let updateOnParams = {};
    if (props.updateOn) {
      const updateValue = props.updateValue?.[props.updateOn];
      if (Array.isArray(updateValue) && updateValue.length > 0) {
        // For arrays, send as JSON string
        updateOnParams[props.updateOn] = JSON.stringify(updateValue);
      } else if (updateValue) {
        // For single values, send as is
        updateOnParams[props.updateOn] = updateValue;
      }
    }
    ...
    <React.Fragment>
      {selectType === "checkbox" ? (
        <ElementContainer ref={selectRef} className={`column box  ${props.customClass}`}>
          ...
          <div
            className={`${selectedId !== null && selectedId.length !== 0 ? "has title" : "title"} ${isLoading ? "loading" : ""}`}
                    ...
                    <Checkbox
                      ...
                      onChange={(event) => {
                        ...
                        if (index === -1) {
                          ...
                          const maxItems = props.maximum ?? Infinity;
        ...
        <div
          className={`${props.dynamicClass?.includes("disabled") ? "hidden" : ""}  col-span-12 rounded-md flex flex-col gap-1 min-w-[200px] w-full ${props.customClass?.includes("filter") ? "w-[max-content] max-w-[100%]" : ""}`}
          role="group"
          aria-label={t(props.label)}
        >
              ...
              <div className="flex items-center gap-4">
                {addNew && addNew.attributes?.length > 0 && (
                  <button
                    ...
                    aria-label={`Add new ${t(props.label)}`}
                  >
          ...
          <div className="relative">
            <Select
              ...
              onChange={(selectedOptions) => {
                ...
                // Check maximum limit
                const maxItems = props.maximum ?? Infinity;
                if (normalizedOptions.length > maxItems) {
                  if (props.setMessage) {
                    props.setMessage({
                      type: 1,
                      content: `Maximum ${maxItems} ${props.label || "item"}${maxItems > 1 ? "s" : ""} allowed`,
            ...
            {hasIcon && (
              <div className={`z-10 absolute left-3 top-[18px] -translate-y-1/2 pointer-events-none ${hasFilter ? "pt-0" : "pt-1"}`}>
                {isLoading ? <Loader className="w-4 h-4 text-gray-500 animate-spin" aria-hidden="true" /> : <GetIcon icon={props.icon} className="w-4 h-4 text-gray-500" aria-hidden="true" />}
              </div>
            )}
          </div>
          <Footnote {...props} />
