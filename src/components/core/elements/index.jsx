...
export const Select = ({
  ...
  align = "",
  apiType = "JSON",
  selectApi = [{ id: "1", value: "No Item Added" }],
  isDisabled = false,
  value = "",
  label = "Select",
  onSelect = () => {},
  ...
  return (
    <FormInput
      ...
      apiType={apiType}
      selectApi={selectApi}
      customClass={`custom ${align} ${customClass}`}
      disabled={isDisabled}
      type="select"
      error={error}
      label={label}
...
export const TabButtons = ({ tabs, selectedTab, selectedChange = () => {}, direct = false, showContentTab, design = "default" }) => {
  ...
  // Design variant two - modern underline style
  if (design === "underline") {
    return (
        ...
        <div className="border-b border-stroke-soft">
          <nav className="-mb-px flex space-x-4" aria-label="Tabs">
            {tabsTemp?.map((tab, index) => (
              <button
                key={tab.key}
                onClick={() => selectedChange(tab.key)}
                className={`py-3 pt-0 text-sm font-medium border-b-2 transition-all duration-200 ease-in-out items-center gap-2 align-center flex px-0 ${
                  selectedTab === tab.key ? "text-primary-base border-blue-600" : "text-gray-500 border-transparent hover:text-gray-700 hover:border-gray-300"
                }`}
              >
                {tab.icon && (
  ...
  // Default design - original styled-components version
  return (
    (tabsTemp?.length >= 1 || (tabsTemp?.length > 0 && direct)) && (
...
export const TextBoxWithButton = ({
  ...
  return (
    <FormInput
      ...
      icon={icon}
      customClass={`custom ${customClass} ${align}`}
      disabled={isDisabled}
      type="buttonInput"
      name="submit"
      footnote={footnote}
      ...
      <TabContainer className="custom tab-menu1">
        ...
        {tabsTemp?.map((tab, index) => (
          <TabButton
            theme={themeColors}
            key={index}
            className={`tab-menu ${selectedTab === tab.key} ${tab.hasErrors ? "has-errors" : ""} ${tab.isCompleted ? "completed" : ""}`}
            onClick={() => selectedChange(tab.key)}
          >
